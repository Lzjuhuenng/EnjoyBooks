...
function getbookMarkTitle1 (epubCli){
    console.log(epubCli);
    var bodyNodes = document.getElementsByTagName('iframe')[0].contentWindow.document.getElementsByTagName('body');

    var Nodes = [].slice.call(bodyNodes[0].childNodes).filter(function(item,index,array){
                return item.nodeType == 1; //&&item.innerHTML.indexOf("<img")
            });
    var str = epubCli.slice(epubCli.lastIndexOf("/",epubCli.lastIndexOf("/")-1)+1,epubCli.lastIndexOf("/"));
    console.log(Nodes);
    //console.log(parseInt(str)/1);
    var nodes = Nodes[parseInt(str)/2-1];
    console.log(nodes);
    while(nodes.nodeType!=3){
        var nodeArr;
        nodeArr = nodes.childNodes; 
        if(nodeArr.length>1){
           nodeArr = [].slice.call(nodes.childNodes).filter(
            function(item,index,array){
                return item.nodeType == 1&&item.nodeName!="img"; 
            });
        }
        nodes = nodeArr[0];
        console.log(nodeArr);
    ...
    console.log(nodes.ownerDocument);
    return nodes.nodeValue.trim().slice(0,20);
